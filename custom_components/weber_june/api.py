"""App-free client for Weber Connect grills over the walker-cloud API.
...
class ApplianceDetails:
    """Model/serial as the cloud reports them. All fields optional."""
...
class WeberJuneClient:
    ...
    def __init__(self, refresh_token: str, appliance_id: str = "",
        ...
        self._after_id = 0
        # Last raw payloads, kept only so diagnostics can show what the cloud
        # actually sends (including fields this client does not decode).
        self.last_snapshot_raw: Optional[dict] = None
        self.last_frame_raw: Optional[bytes] = None
    ...
    def appliance_details(self) -> ApplianceDetails:
        """Model/serial for this appliance, or empty details when unavailable.

        The cloud only exposes this per companion registration, so it needs the
        companion id; without one (or on any failure) the caller falls back to
        static device info rather than losing the entities.
        """
        if not self.companion_id:
            return ApplianceDetails()
        try:
            code, body = self._get_retrying(
                f"{API_BASE}/2/devices/{self.companion_id}/associated")
        except WeberConnectionError:
            return ApplianceDetails()
        if code != 200 or not isinstance(body, dict):
            return ApplianceDetails()
        for dev in body.get("devices") or []:
            if isinstance(dev, dict) and dev.get("oven_id") == self.appliance_id:
                return ApplianceDetails(
                    name=dev.get("name") or None,
                    model_number=dev.get("model_number") or None,
                    serial_number=dev.get("serial_number") or None,
                )
        return ApplianceDetails()
    ...
    @staticmethod
    def frame_inventory(frame: bytes) -> list[dict]:
        """Every TLV in a companion frame, decoded or not.

        `_parse_frame` deliberately reads only the handful of fields the
        entities need; this lists all of them so unknown fields are visible
        ...
        while j + 2 <= len(body):
            f, ln = body[j], body[j + 1]
            if j + 2 + ln > len(body):
                items.append({"field": f"0x{f:02x}", "error": "truncated"})
                break
            val = body[j + 2:j + 2 + ln]
            item: dict = {"field": f"0x{f:02x}", "len": ln, "hex": val.hex()}
            if ln in (1, 2, 4):
                item["as_int_le"] = int.from_bytes(val, "little")
                if ln == 2:
                    item["as_deci_c"] = round(item["as_int_le"] / DECI, 1)
            if f == 0x04:
                sub: list[dict] = []
                k = 0
                while k + 2 <= len(val):
                    pf, pln = val[k], val[k + 1]
                    if k + 2 + pln > len(val):
                        sub.append({"field": f"0x{pf:02x}", "error": "truncated"})
                        break
                    pv = val[k + 2:k + 2 + pln]
                    s: dict = {"field": f"0x{pf:02x}", "len": pln, "hex": pv.hex()}
                    if pln in (1, 2, 4):
                        s["as_int_le"] = int.from_bytes(pv, "little")
                        if pln == 2:
                            s["as_deci_c"] = round(s["as_int_le"] / DECI, 1)
                    sub.append(s)
                    k += 2 + pln
                item["sub"] = sub
            items.append(item)

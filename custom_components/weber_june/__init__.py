"""The Weber Connect (cloud, app-free) integration."""
...
async def async_setup_entry(hass: HomeAssistant, entry: WeberJuneConfigEntry) -> bool:
    """Set up Weber Connect from a config entry."""
    ...
    client = WeberJuneClient(
        ...
        companion_id=entry.data.get(CONF_COMPANION_ID, ""),
    )
    coordinator = WeberJuneCoordinator(hass, entry, client)
    await coordinator.async_config_entry_first_refresh()
    await coordinator.async_load_details()
